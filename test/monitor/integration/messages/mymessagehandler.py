...
class MessageHandler1(AbcMessageHandler):
    def handle_messages(self, message):
        if message.topic == "testTopic4":
            print(
                f"handler1: Topic: {message.topic} Partition: {message.partition} Offset: {message.offset} Message: {message.value}"
            )
...
class MessageHandler2(AbcMessageHandler):
    def handle_messages(self, message):
        if message.topic == "testTopic5":
            print(
                f"handler2: Topic: {message.topic} Partition: {message.partition} Offset: {message.offset} Message: {message.value}"
            )
